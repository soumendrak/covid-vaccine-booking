...
def check_calendar(
    request_header, vaccine_type, district_dtls, minimum_slots, min_age_booking
):
    """
    This function
    ...
    try:
        ...
        for district in district_dtls:
            resp = requests.get(
                base_url.format(district["district_id"], tomorrow),
                ...
                if len(resp["centers"]) >= 0:
                    for center in resp["centers"]:
                        for session in center["sessions"]:
                            if (session["available_capacity"] >= minimum_slots) and (
                                session["min_age_limit"] <= min_age_booking
                            ):
                                out = {
                                    "name": center["name"],
...
def generate_token_OTP(mobile):
    ...
    txnId = requests.post(
        url="https://cdn-api.co-vin.in/api/v2/auth/generateMobileOTP", json=data
    )
    ...
    token = requests.post(
        url="https://cdn-api.co-vin.in/api/v2/auth/validateMobileOtp", json=data
    )
